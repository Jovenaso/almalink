...
public class GralEmpresaDaoImpl {
	@Autowired
	...
	public BusquedaListaPaginar busqueda(Map<String, Object> conditions) {
		...
		CriteriaQuery<GralEmpresaDto> criteria = builder.createQuery(GralEmpresaDto.class);
		Root<GralEmpresa> root = criteria.from(GralEmpresa.class);
		criteria.multiselect(root.get("id"), 
				root.get("nombre"), 
				root.get("nombre_comercial"), 
				root.get("rfc"), 
				root.get("direccion"), 
				root.get("logo"));
		...
		List<GralEmpresaDto> result = entityManager.createQuery(criteria).setFirstResult(0)
				.setMaxResults(1000).getResultList();
		CriteriaQuery<Long> countQuery = builder.createQuery(Long.class);
		Root<GralEmpresa> rootCount = countQuery.from(GralEmpresa.class);
