...
public class GralColorDaoImpl {
	...
	public BusquedaListaPaginar busqueda(Map<String, Object> conditions) {
		...
		CriteriaQuery<GralColorDto> criteria = builder.createQuery(GralColorDto.class);
		Root<GralColor> root = criteria.from(GralColor.class);
		...
		List<GralColorDto> result = entityManager.createQuery(criteria).setFirstResult(0)
				.setMaxResults(1000).getResultList();
		CriteriaQuery<Long> countQuery = builder.createQuery(Long.class);
		Root<GralColor> rootCount = countQuery.from(GralColor.class);
